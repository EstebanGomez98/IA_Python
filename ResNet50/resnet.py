...
def recognize_image():
    ...
    if file_path:
        ...
        class_labels = [class_names[top5_catid[i]]
                        for i in range(top5_prob.size(0))]

        # Display the recognition results
        results_label.config(text="\nTop 5 clases mas reconocidas:")
        for i in range(top5_prob.size(0)):
            results_label.config(text=results_label.cget(
...
recognize_button = Button(
    root, text="RECONOCER IMAGEN", command=recognize_image)
...
with open("D:\Biblioteca\Documentos\IA_Python\ResNet50\imagenet-classes.txt") as f:
